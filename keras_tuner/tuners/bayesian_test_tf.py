...
from keras_tuner.tuners import bayesian_tf as bo_module
...
def test_bayesian_oracle(tmp_path):
    ...
    oracle = bo_module.BayesianOptimizationOracleTF(
...
def test_bayesian_optimization_tuner(tmp_path):
    tuner = bo_module.BayesianOptimizationTF(
        build_model, objective="val_accuracy", max_trials=15, directory=tmp_path
    )
    assert isinstance(tuner.oracle, bo_module.BayesianOptimizationOracleTF)
